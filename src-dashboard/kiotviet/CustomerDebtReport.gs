...
function syncCustomerDebtReports(token) {
  ...
  try {
    ...
    const transactions = buildCustomerDebtTransactions_(
      customers, invoices, returns, receipts.concat(expenses), productLookup, maxPeriod.start.getTime()
    );
...
function buildCustomerDebtTransactions_(customers, invoices, returns, cashFlows, productLookup, windowStartMs) {
  ...
  addCustomerDebtInitializationAdjustments_(lookup, transactions, windowStartMs);
 ...
 */
function addCustomerDebtInitializationAdjustments_(lookup, transactions, windowStartMs) {
  ...
  (lookup.profiles || []).forEach(profile => {
    ...
    // Chi khoi tao but toan CB cho khach moi tao trong dung cua so du lieu da
    // lay (7 ngay); khach no cu ngoai cua so nay khong co du lieu de xac dinh
    // dung thoi diem phat sinh no, tranh gan nham vao ngay sua ho so gan nhat.
    if (profile.createdTimeMs < windowStartMs) return;
...
function aggregateCustomerDebtReport_(customers, transactions, period) {
  const states = {};
  const lookup = buildCustomerDebtProfileLookup_(customers);
  // Khong khoi tao truoc state cho moi khach con no (closingDebt !== 0): lam
  // vay se dua ca khach no cu, khong co giao dich nao trong ky, vao ca 3 sheet
  // HN1/HN3/HN7 giong het nhau. Chi nhung khach co it nhat 1 giao dich (ban
  // hang/tra hang/thu chi/dieu chinh) roi vao dung ky moi duoc dua vao bao cao.
...
/**
 * Moi dong la mot giao dich (hoac mot mat hang trong giao dich, neu giao dich
 * co nhieu mat hang) thay vi don het cac gia tri chi tiet vao 1 o ngan cach
 * boi dau |. Cac cot thong tin khach hang duoc lap lai tren tung dong de
 * moi dong van loc/sap xep doc lap va khong lam mat ngu canh khach hang.
 */
function buildCustomerDebtReportValues_(rows, period) {
  ...
  (rows || []).forEach(row => {
    ...
    const customerInfo = [
      row.profile.code, row.profile.name, row.profile.contactNumber, row.profile.group,
      row.openingDebt, row.debit, row.credit, row.closingDebt
    ];

    transactions.forEach(transaction => {
      const reportLines = buildCustomerDebtTransactionReportLines_(transaction);
      reportLines.forEach(reportLine => {
        const productLine = reportLine.productLine;
        values.push(customerInfo.concat([
          transaction.code,
          customerDebtDateTimeText_(transaction.time),
          transaction.type,
          reportLine.value,
...
/**
 * Khi mot hoa don co nhieu mat hang, cot Gia tri phai phan anh thanh tien cua
 * tung dong thay vi lap lai tong hoa don. So du duoc cong don theo tung dong;
 * chenhlech cap hoa don (giam gia/thu khac/VAT) duoc dua vao dong cuoi de tong
 * cac dong va so du cuoi van khop chinh xac voi giao dich tren KiotViet.
 */
function buildCustomerDebtTransactionReportLines_(transaction) {
  const productLines = transaction.productLines && transaction.productLines.length > 0
    ? transaction.productLines
    : [];
  const transactionValue = customerDebtNumber_(transaction.value);
  const transactionRunningDebt = customerDebtNumber_(transaction.runningDebt);
  if (productLines.length === 0) {
    return [{
      productLine: null,
      value: transactionValue,
      runningDebt: transactionRunningDebt
    }];
  }

  let runningDebt = transactionRunningDebt - transactionValue;
  let allocatedValue = 0;
  return productLines.map((productLine, index) => {
    const isLastLine = index === productLines.length - 1;
    const lineValue = isLastLine
      ? transactionValue - allocatedValue
      : customerDebtNumber_(productLine.amount);
    allocatedValue += lineValue;
    runningDebt += lineValue;
    return {
...
function writeCustomerDebtReportSheet_(sheetName, rows, period, syncedAt) {
  ...
  sheet.getRange(1, 1).setNote(
    ...
    'Moi giao dich/mat hang hien thi tren mot dong rieng (thong tin khach hang lap lai o dau dong).\n' +
