...
| 3     | GAS Web App (src/)                    | Multi-file GAS: Auth, SheetSchemas, CustomerDebtReport, CustomerReport, SyncInitial, WebhookAdmin, UpdateHandlers, WebhookQueue, DashboardData, WebApp | ✅ Hoàn thành  |
...
> **Schema Google Sheets:** Apps Script duy trì 9 tab vận hành, tab `Báo cáo bán hàng` 18 cột theo từng giao dịch trong tháng hiện tại, tab `Hàng bán theo khách` đúng 5 cột chi tiết hàng bán trong 90 ngày và ba tab công nợ `HN1`/`HN3`/`HN7` dùng 25 cột của file xuất công nợ KiotViet nhưng chỉ tạo một dòng cho mỗi khách hàng (nhiều giá trị chi tiết được ghép bằng dấu `|`). Webhook hóa đơn cập nhật `Hàng bán theo khách` theo chu kỳ hàng đợi 1 phút; lượt 07:00 đối soát hai báo cáo bán hàng và lượt 15:00 đối soát ba báo cáo công nợ. `src/kiotviet/SheetSchemas.gs` giữ cột dashboard ở bên trái, chỉ nối các trường KiotViet dạng phẳng đang dùng và tự xóa cột JSON cũ qua trigger nền. Backend dashboard chỉ đọc 9 tab vận hành và dùng `Nhóm hàng` để gom tồn kho theo nhóm cha.
