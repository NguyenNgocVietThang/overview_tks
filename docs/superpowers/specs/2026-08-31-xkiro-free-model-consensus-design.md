# Design: Đồng thuận 2/5 model xKiro cho nhận dạng tin nhắn xin nghỉ
...
Tăng độ tin cậy và giảm thời gian chờ của bước AI extraction bằng cách gọi song song năm model free trên cùng xKiro API. Bot sử dụng kết quả ngay khi hai model trả kết quả hợp lệ và khớp nhau về ý định cùng khoảng nghỉ đã được resolver xác nhận. Không có cặp đồng thuận thì không ghi Google Sheet.

## 2. Model và cấu hình

Thêm biến tùy chọn `AI_LEAVE_API_MODELS`, nhận danh sách model cách nhau bằng dấu phẩy. Bộ năm model triển khai ban đầu:

1. `deepseek/deepseek-v4-pro`
2. `deepseek/deepseek-v4-flash`
3. `qwen/qwen3-max:free`
...
`leaveAiExtractor` tiếp tục chịu trách nhiệm gọi một model và xác thực JSON schema. Một module consensus mới điều phối năm lời gọi song song, đo latency nội bộ và không log response thô.
...
Ngay khi có hai phiếu đồng thuận, hệ thống trả kết quả và hủy các request còn lại nếu còn chạy. Hai phiếu có thể đến từ bất kỳ cặp model nào trong năm model; hệ thống không chờ đa số tuyệt đối. Một model lỗi, timeout, response sai schema, confidence thấp hoặc resolver từ chối không được tính là phiếu.
...
- Hai model đồng thuận sớm: trả kết quả, không chờ ba model còn lại.
- Các response đầu bất đồng, một response đến sau tạo thành cặp khớp và quyết định kết quả.
...
- Không đạt 2/5 trả `AI_LEAVE_NO_CONSENSUS` và không ghi Sheet.
- Fallback một model khi `AI_LEAVE_API_MODELS` trống.
- Không làm lộ key/response thô trong lỗi hoặc log.

Sau khi test mock đạt, smoke test thật gọi năm model bằng tin nhắn không nhạy cảm để xác nhận availability, latency và consensus. Sau đó khởi động bot polling nền; người dùng thực hiện smoke test Telegram và xác nhận để ghi một dòng Sheet thật.
...
- Năm request được gọi song song và có thể kết thúc sớm khi đạt một cặp 2/5 khớp nhau.
