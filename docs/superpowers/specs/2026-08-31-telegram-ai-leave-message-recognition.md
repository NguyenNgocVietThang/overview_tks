...
## 3. Bối cảnh kỹ thuật hiện hành

- Bot chạy Node.js 22 trên VPS bằng long polling (`node-telegram-bot-api`), không cần webhook công khai và nằm trong repository/deployment riêng; không phụ thuộc web runtime/Render.
- Bot sở hữu business logic ngày/buổi, AI resolver, xác thực danh tính, Google Sheets repository và decision watcher của chính nó; không import module từ `server/`.
- Bot đọc danh sách nhân sự/mã liên kết, ghi đơn nghỉ và quét trạng thái duyệt trực tiếp qua Google Sheets API bằng service account.
- Trạng thái hội thoại theo `chat_id` được lưu bền vững bằng file JSON trên VPS và có thể sao lưu vào Google Sheets, sống sót qua restart.
- Secret bot (`TELEGRAM_BOT_TOKEN`, `AI_LEAVE_*`) chỉ nằm trong `.env` của deployment VPS. Web giữ cấu hình riêng trong `server/config.js` và `server/.env`/Render.
- Repository web chỉ quản lý giao diện/API HR và cập nhật trạng thái trên Sheets; giao tiếp giữa web và bot hoàn toàn qua dữ liệu Google Sheets.
